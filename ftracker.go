...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed(action, duration)*kmhInMsec, float64(2)) / (height / cmInM) * walkingSpeedHeightMultiplier * weight)) * duration * minInH)
}
