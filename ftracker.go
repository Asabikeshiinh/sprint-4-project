...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Плавание":
		distance := float64(float64(lengthPool) * float64(countPool) / mInKm)      // вызовите здесь необходимую функцию
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed(action, duration)*kmhInMsec, float64(2))/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
